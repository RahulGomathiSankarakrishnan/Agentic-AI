from Backend_4_RAG import (chatbot, create_table, retrieve_all_threads, retrieve_all_summaries, save_summary, ingest_pdf, thread_document_metadata, delete_thread)
...
with st.sidebar:
    ...
    if uploaded_pdf:
        # Check if already indexed via backend metadata check
    ...
    for item in st.session_state['summary_history']:
        ...
        with col2:
            # Use a trash emoji or icon
            if st.button("🗑️", key=f"del_{t_id}", help="Delete this chat"):
                if delete_thread(t_id):
                    # If we deleted the active thread, reset the view
...
if user_input:
    ...
    with st.chat_message("assistant"):
        config = {'configurable': {'thread_id': thread_key},'run_name':'chat_turn','metadata':{'thread_id':thread_key}}
    ...
    st.session_state['summary_history'] = retrieve_all_summaries()
    # st.session_state["ingested_docs"].setdefault(str(thread_key), {})
    # print(st.session_state["ingested_docs"])
    st.rerun()
